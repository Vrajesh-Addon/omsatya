...
import 'package:omsatya/screen/admin/dash_home.dart';
...
class _DashboardState extends State<Dashboard> {
  ...
  final drawerItems = [
    ...
    DrawerItem(AppString.addComplain, Icons.warning_amber_rounded),
    DrawerItem(AppString.dailyAttendance, Icons.person_outline_rounded),
    DrawerItem(AppString.locationAddress, Icons.apartment_rounded),
    DrawerItem(AppString.todo, Icons.person_rounded),
    DrawerItem(AppString.sales, Icons.person_rounded),
    DrawerItem(AppString.leave, Icons.calendar_month_rounded),
    DrawerItem(AppString.addAttendance, Icons.co_present_outlined),
    DrawerItem(AppString.adminTodayReport, Icons.picture_as_pdf_rounded),
  ...
  init() async {
    ...
    //Hide
    if (AppGlobals.user!.phoneNo != "9913750000") {
      drawerItems.removeWhere((item) => item.title == AppString.addAttendance);
      setState(() {});
    }
  }
  ...
  _getDrawerItemWidget(BuildContext context, int pos) {
    switch (pos) {
      ...
      case 2:
        return AppGlobals.navigate(context, AdminAddComplain(
          changeIndex: (index) {
            _onSelectChangeIndex(index);
          },
        ), false);
      case 3:
        return AppGlobals.navigate(context, const DailyAttendance(), false);
      case 4:
        return AppGlobals.navigate(context, const AddPartyAddress(), false);
      case 5:
        return AppGlobals.navigate(context, const Todo(), false);
      case 6:
        return AppGlobals.navigate(context, const Sales(isAdmin: true), false);
      case 7:
        return AppGlobals.navigate(context, const LeaveScreen(), false);
      case 8:
        if (AppGlobals.user!.phoneNo != "9913750000") {
          return AppGlobals.navigate(context, const AdminTodayReport(), false);
        }
        if (AppGlobals.user!.phoneNo == "9913750000") return AppGlobals.navigate(context, const AddAttendance(), false);
  ...
  _onSelectChangeIndex(int index) {
    ...
    if (index == 3) {
      AppGlobals.navigate(context, const DailyAttendance(), false);
    } else if(index == 7) {
      AppGlobals.navigate(context, const LeaveScreen(), false);
    } else {
      AppGlobals.navigate(context, const AdminTodayReport(), false);
    }
    if (_scaffoldKey.currentState!.isDrawerOpen) {
      Navigator.of(context).pop(); // close the drawer
    }
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: DashHomeScreen(
          ...
          changeIndex: (index) => _onSelectChangeIndex(index),
        ),
      ),
    );
  }
